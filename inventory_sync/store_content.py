...
Currently the fixed delivery/returns boilerplate that every ingested product
carries in its `custom.delivery` metafield — the same text for Laura and Segal
(owner instruction: "3 — to all").
"""
...
# Owner-supplied fixed shipping + returns text. Handles furniture explicitly.
...
def delivery_metafield() -> Metafield:
    """The shared custom.delivery metafield (same for every supplier)."""
    return Metafield("custom", "delivery", RICH_TEXT, rich_text_paragraphs(DELIVERY_BOILERPLATE_LINES))
