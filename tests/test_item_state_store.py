...
class ItemStateStoreContract:
    ...
    def test_fresh_store_returns_empty_set(self, store: ItemStateStore):
        assert store.get_active_skus("laura", "unarchive_candidate") == set()

    def test_fresh_store_is_not_seeded(self, store: ItemStateStore):
        assert store.is_seeded("laura", "unarchive_candidate") is False

    # --- set_active basic behavior ---

    def test_set_active_writes_skus(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"A", "B", "C"})
        assert store.get_active_skus("laura", "unarchive_candidate") == {"A", "B", "C"}

    def test_set_active_marks_as_seeded(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", set())
        assert store.is_seeded("laura", "unarchive_candidate") is True

    def test_set_active_marks_seeded_even_for_non_empty_set(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"X"})
        assert store.is_seeded("laura", "unarchive_candidate") is True

    def test_set_active_replaces_prior_set_entirely(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"A", "B"})
        store.set_active("laura", "unarchive_candidate", {"B", "C"})
        assert store.get_active_skus("laura", "unarchive_candidate") == {"B", "C"}
    ...
    def test_seeded_persists_even_when_active_set_emptied(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"A"})
        store.set_active("laura", "unarchive_candidate", set())
        assert store.is_seeded("laura", "unarchive_candidate") is True
    ...
    def test_different_vendors_do_not_collide(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"X"})
        store.set_active("snir", "unarchive_candidate", {"X", "Y"})
        assert store.get_active_skus("laura", "unarchive_candidate") == {"X"}
        assert store.get_active_skus("snir", "unarchive_candidate") == {"X", "Y"}

    def test_clearing_one_vendor_leaves_other_intact(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"X"})
        store.set_active("snir", "unarchive_candidate", {"X"})
        store.set_active("laura", "unarchive_candidate", set())
        assert store.get_active_skus("snir", "unarchive_candidate") == {"X"}
        assert store.is_seeded("snir", "unarchive_candidate") is True

    def test_different_state_keys_do_not_collide(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"X"})
        store.set_active("laura", "oos", {"Y"})
        store.set_active("laura", "unarchive_candidate", set())
        assert store.get_active_skus("laura", "oos") == {"Y"}
        assert store.is_seeded("laura", "oos") is True

    def test_seeded_flag_is_per_vendor_and_per_key(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"A"})
        assert store.is_seeded("laura", "unarchive_candidate") is True
        assert store.is_seeded("laura", "oos") is False
        assert store.is_seeded("snir", "unarchive_candidate") is False

    # --- bulk / scale smoke ---

    def test_can_store_large_active_set(self, store: ItemStateStore):
        big = {f"SKU-{i:06d}" for i in range(1000)}
        store.set_active("laura", "unarchive_candidate", big)
        assert store.get_active_skus("laura", "unarchive_candidate") == big

    def test_returned_set_is_independent_of_store_state(self, store: ItemStateStore):
        store.set_active("laura", "unarchive_candidate", {"A", "B"})
        result = store.get_active_skus("laura", "unarchive_candidate")
        result.add("Z")
        # Subsequent read must not reflect the caller's mutation
        assert store.get_active_skus("laura", "unarchive_candidate") == {"A", "B"}
